...
# Generate an Ed25519 signing keypair for pisweep release signing.
#
...
OUT_DIR="${1:-$HOME/.pisweep/signing}"

usage() {
    cat <<EOF
Usage: $0 [OUT_DIR]

Generate an Ed25519 keypair for signing pisweep releases.

  OUT_DIR  Directory to write the keys (default: \$HOME/.pisweep/signing)
...
in the netinstaller (or the pisweep-signing-key.pub file).
EOF
}
