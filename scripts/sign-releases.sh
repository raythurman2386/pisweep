...
# ~/.pisweep/signing/releases/<version>/checksums.txt.sig. The checksums.txt is
...
SIGNING_DIR="${PISWEEP_SIGNING_DIR:-$HOME/.pisweep/signing}"
SECRET_KEY="${PISWEEP_SIGNING_KEY:-$SIGNING_DIR/secret.pem}"
...
Environment:
  PISWEEP_SIGNING_DIR   Override the signing directory (default: ~/.pisweep/signing)
  PISWEEP_SIGNING_KEY   Override the secret key path
EOF
}
